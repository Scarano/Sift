...
public class LogSum {
    /**
     * If x / y > exp(CHEAT_THRESHOLD), then logSum(lx, ly) short-circuits to lx.
     ...
     * results.
     */
    static final double CHEAT_THRESHOLD = 9.0;
    ...
    public static double logSum(double lx, double ly) {
        ...
        double d = lx - ly;
        if (d >= 0) {
            if (d > CHEAT_THRESHOLD) return lx;
            else return lx + Math.log1p(Math.exp(-d));
        }
        else {
            if (d < -CHEAT_THRESHOLD) return ly;
